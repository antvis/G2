...
describe('#1184', () => {
  ...
  it('cannot render label when polygon is defined by category fields', () => {
    const data = [
      [ 0, 0, 10 ],
      [ 0, 1, 19 ],
      [ 0, 2, 8 ],
    ...
    const labelContainer = chart.getAllGeoms()[0].get('labelContainer');
    ...
    G2.Util.each(labels, label => {
      expect(label.attr('x') > 0 && label.attr('x') < width).to.be.true;
      expect(label.attr('y') > 0 && label.attr('y') < height).to.be.true;
    });
  });
});
