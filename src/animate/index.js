...
function cache(shapes) {
  ...
  Util.each(shapes, shape => {
    ...
    rst[id] = {
      _id: id,
      type: shape.get('type'),
      attrs: Util.cloneDeep(shape._attrs), // 原始属性
      name: shape.name,
...
function addAnimate(cache, shapes, canvas, isUpdate) {
  ...
  if (isUpdate) {
    ...
    Util.each(updateShapes, updateShape => {
      ...
      // 判断如果属性相同的话就不进行变换
      if (!Util.isEqual(cacheAttrs, updateShape._attrs)) {
        ...
        if (Util.isFunction(animate)) {
          ...
          const endState = Util.cloneDeep(updateShape._attrs);
          // updateShape._attrs = cacheAttrs;
