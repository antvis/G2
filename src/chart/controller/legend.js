...
import Tail from '../../component/legend/tail';
...
class LegendController {
  ...
  _getXAlign(pos, width, region, backRange, legendWidth, borderMargin) {
    const leftPos =
      backRange.minX - legendWidth - borderMargin[3] < 0
        ? 0
        : backRange.minX - legendWidth - borderMargin[3];
  ...
  _getYAlignHorizontal(pos, height, region, backRange, legendHeight, borderMargin) {
    const y =
      pos === 'top'
        ? backRange.minY - legendHeight - borderMargin[0]
        : backRange.maxY + borderMargin[2];
    return y;
  }

  _getYAlignVertical(pos, height, region, backRange, legendHeight, borderMargin, canvasHeight) {
    let y =
      pos === 'top' ? backRange.minY - legendHeight - borderMargin[0] : height - region.totalHeight;
    if (pos === 'center') {
  ...
  _addCategoryLegend(scale, attr, geom, filterVals, position) {
    ...
    const posArray = position.split('-');
    const maxLength =
      posArray[0] === 'right' || posArray[0] === 'left'
        ? plotRange.bl.y - plotRange.tr.y
        : canvas.get('width');
    ...
    if (self._isTailLegend(legendOptions, geom)) {
      ...
      if (legendOptions.useHtml) {
        ...
        if (legendOptions.flipPage) {
          legendCfg.legendStyle.CONTAINER_CLASS.height =
            posArray[0] === 'right' || posArray[0] === 'left' ? maxLength + 'px' : 'auto';
          legendCfg.legendStyle.CONTAINER_CLASS.width = !(
            posArray[0] === 'right' || posArray[0] === 'left'
          )
            ? maxLength + 'px'
            : 'auto';
  ...
  _addContinuousLegend(scale, attr, position) {
    ...
    if (
      (options && options.slidable === false) ||
      (options[field] && options[field].slidable === false)
    ) {
  ...
  addLegend(scale, attr, geom, filterVals) {
    ...
    if (fieldOption && fieldOption.custom) {
      ...
      if (fieldOption && fieldOption.position) {
        ...
        position = self._adjustPosition(
          fieldOption.position,
          self._isTailLegend(fieldOption, geom)
        );
      }
      let legend;
      if (scale.isLinear) {
