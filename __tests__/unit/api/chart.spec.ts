...
import { ChannelGroups } from '../../../src/runtime';
...
describe('Chart', () => {
  ...
  it('get instance information after chart render.', (done) => {
    ...
    setTimeout(() => {
      const context = chart.context();
      expect(chart.getView()).toEqual(
        context.views?.find((v) => v.key === chart.attr('key')),
      );

      expect(chart.getGroup()).toEqual(context.canvas?.getRoot());
