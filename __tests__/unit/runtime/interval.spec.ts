...
describe('render', () => {
  it.skip('render({}) returns a canvas wrapped in HTMLElement with default size', () => {
    const chart = render({
      type: 'interval',
    });
  ...
  it.skip('render({width, height}) returns a canvas wrapped in HTMLElement with specified size', () => {
    const chart = render({
      width: 800,
      height: 200,
      type: 'interval',
  ...
  it('render({...} renders interval chart with funnel shape when symmetryY', () => {
    const chart = render<G2Spec>({
      type: 'interval',
  ...
  it('render({...} renders interval chart with pyramid shape when symmetryY', () => {
    const chart = render<G2Spec>({
      type: 'interval',
      transform: [{ type: 'symmetryY' }],
      data: [
