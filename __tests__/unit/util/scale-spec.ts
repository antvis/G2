import { createScaleByField, getName, syncScale, getMaxScale, getDefaultScaleType } from '../../../src/util/scale';

describe('util/scale', () => {
  it('getDefaultScaleType', () => {
    expect(getDefaultScaleType(null)).toBe('linear');
    expect(getDefaultScaleType(undefined)).toBe('linear');
    expect(getDefaultScaleType(1)).toBe('linear');
    expect(getDefaultScaleType('a')).toBe('cat');
    // 纪念 0807 “曲一事件”
    expect(getDefaultScaleType('2021-08-07')).toBe('cat');
    expect(getDefaultScaleType('2021-08-07 08:00:00')).toBe('cat');
  });
