...
import * as chartTests from '../plots/legend';
  ...
  for (const [name, generateOptions] of tests) {
    ...
    it(`[Legend]: ${name}`, async () => {
      try {
        ...
        // @ts-ignore
        const { className = 'legend-category' } = generateOptions;
        // @ts-ignore
        ...
        const dir = `${__dirname}/snapshots/legend/${kebabCase(name)}`;
